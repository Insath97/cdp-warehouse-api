...
class UpdateSupplierRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            // Nested Bank accounts validation for update
            'bank_accounts' => 'nullable|array',
            'bank_accounts.*.id' => 'sometimes|integer|exists:supplier_bank_accounts,id',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (is_array($bankAccounts)) {
                $primaryCount = 0;
                foreach ($bankAccounts as $index => $acct) {
                    if (isset($acct['is_primary']) && filter_var($acct['is_primary'], FILTER_VALIDATE_BOOLEAN)) {
                        $primaryCount++;
                    }

                    // If an ID is provided, verify it belongs to the current supplier
                    if (isset($acct['id'])) {
                        $belongs = \DB::table('supplier_bank_accounts')
                            ->where('id', $acct['id'])
                            ->where('supplier_id', $supplierId)
                            ->exists();
                        if (!$belongs) {
                            $validator->errors()->add("bank_accounts.{$index}.id", "The selected bank account does not belong to this supplier.");
                        }
                    }
                }
                if ($primaryCount > 1) {
